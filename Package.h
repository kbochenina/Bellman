...
class Package
{
	...
	map <pair <int,int>, int> execTime; // ((1,2,15)) means that package executes 15 seconds on 2 cores of restype1
	
public:
	Package(int u, vector <int> &r, vector <int> &c, map <pair <int,int>, int> &e);
	...
	int GetExecTime(int type, int cores){ return execTime[std::make_pair(type,cores)];}
